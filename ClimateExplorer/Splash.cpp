...
(
	Gdiplus::Graphics& graphics, PointF pointText,
	Gdiplus::Font& largeFont, Gdiplus::Font& smallFont, Gdiplus::Font& tinyFont,
	int nLargeHeight, int nSmallHeight, int nTinyHeight,
...
)
{
	...
	pointText.Y += nLargeHeight + 10;

	// Draw small text
	CClimateExplorerApp* pApp = (CClimateExplorerApp*)AfxGetApp();

	CString csVersion = pApp->DatabaseVersion;
	csVersion.Format(L"Database Version: %s", csVersion);
	graphics.DrawString
	(
		csVersion, -1,
	...
	pointText.Y += 2 * nSmallHeight + 5;

	CString csDesc = m_version.FileDescription;

	graphics.DrawString
	(
		csDesc, -1, &smallFont, pointText, &format, &brush
	);
	...
	pointText.Y += nSmallHeight + 20;

	CString csCredits = pApp->DatabaseCredits;
	csVersion.Format(L"%s", csCredits);
	graphics.DrawString
	(
		csCredits, -1,
		&tinyFont, pointText, &format, &brush
	);
...
void CSplash::DrawText( CPaintDC& dc )
{
	...
	Gdiplus::Font tinyFont
	(
		&fontFamily, REAL(nTinyHeight),
		Gdiplus::FontStyleBold, Gdiplus::UnitPixel
	);
	...
	DrawLabels
	(
		graphics, pointText, largeFont, smallFont, tinyFont,
		nLargeHeight, nSmallHeight, nTinyHeight, brushBottom, format
	);
	...
	(
		graphics, pointText, largeFont, smallFont, tinyFont,
		nLargeHeight, nSmallHeight, nTinyHeight, brushTop, format
	);
